...
<script type="text/javascript">
	...
	function formatterOperation(val, row) {
		url='${ctx}/admin/audit/finalForAuditingDetailPageJsp?userId='+row.userId;
		return '<a href="'+url+'">操作</a>';
	}
	
	function search(){
		var queryParams = $('#finalAuditTbl').datagrid('options').queryParams;
		...
		$('#finalAuditTbl').datagrid('options').queryParams = queryParams;
		$("#finalAuditTbl").datagrid('reload');
	}
...
<body>
<table id="finalAuditTbl" title="终定列表" class="easyui-datagrid"
			url="${ctx}/admin/audit/waitAuditUserPage?auditId=2"
	...
	<thead>
		<tr>
			...
			<th field="tempCreidtGrade" sortable="true">审核中信用等级</th>
			<th field="tempCreditAmount" >审核中信用额度</th>
