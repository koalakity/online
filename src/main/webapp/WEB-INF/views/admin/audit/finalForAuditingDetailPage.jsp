...
<script type="text/javascript">
 ...
 var exam = [
	...
	function confirm1(){
	          var amount = $.trim($('#creditAmountTxt').val());
	          var dataAry = new Array();
	          $("input[name='creditGrade']").each(function(i){
		         var data = new Object();
		         var proID = this.id;
		         var creditScore = this.value;
		         data.proId = proID;
		         data.creditScore=creditScore;
		         data.reviewStatus="0";
		         dataAry.push(data);
		      })
		      $("select[name='cardSel']").each(function(i){
		          var id = (this.id).substring(3,this.id.length);
		          for(var j=0;j<dataAry.length;j++){
                      if(dataAry[j].proId==id){
                         dataAry[j].reviewStatus=this.value;
                      }
		          }
		      })
		        var creditAmount = $.trim($("#creditAmount").val());
		        $.messager.confirm('确定', '审核通过，您确定要提交吗？',function(r){
		         if(r){
		           $.post(
		         "${ctx}/admin/audit/auditPass?userId=${userAuditInfo.userId}&auditId=5&creditAmount="+amount,
		          {
		           dataAry : JSON.stringify(dataAry)
		         },
		         function(data,textStatus){
		           location.href='${ctx}/admin/audit/finalForAuditingUserPageJsp';
		         },"json");
		         }
		      });
	}
	...
	function prompt1(){
		$('<div/>').dialog({
			href : '${ctx}/static/admin/reject.html',
			width : 350,
			height : 180,
			modal : true,
			title : '驳回',
					 ...
					 $.post(
				         ...
				         function(data){
				           ...
				           if(data.success==true){
				             location.href='${ctx}/admin/audit/waitForAuditingUserPageJsp';
				           }
				         },"json");
  ...
  function alertWarn(){
			$.messager.alert('警告','补充资料说明不能为空！','warning');
		}
	function prompt2(){
		$('<div/>').dialog({
			href : '${ctx}/static/admin/replenish.html',
			width : 350,
			height : 180,
			modal : true,
			title : '补充资料',
			buttons : [{
				...
				iconCls : 'icon-ok',
				handler : function(){
					var replenishText=$("#replenishText").val();
					if(replenishText==""){
					  alertWarn();
					  return false;
					}
					...
					$.messager.show({
					 ...
					 $.post(
				         "${ctx}/admin/audit/auditReplenish",
				         {
				           userId :${userAuditInfo.userId},
				           cause   : replenishText,
				           operateNode : '待审'
	...
	function prompt3(){
	   $.messager.prompt('退回', '退回后此信息将进入被退回节点列表，请填写退回的：',function(r){
		  if(r){
		      $.post(
		         "${ctx}/admin/audit/auditReturnToWait",
		          {
		           userId :${userAuditInfo.userId},
		           cause   : r,
		           operateNode : '终定'
	...
	function prompt4(){
	   $.messager.prompt('退回', '退回后此信息将进入被退回节点列表，请填写退回的：',function(r){
		  if(r){
		      $.post(
		         "${ctx}/admin/audit/auditReturnToFirst",
		          {
		           userId :${userAuditInfo.userId},
		           cause   : r,
		           operateNode : '终定'
	...
	function save(){
	     var dataAry = new Array();
		  ...
		  $.post(
		         "${ctx}/admin/audit/auditSave?userId=${userAuditInfo.userId}",
		          {
		           dataAry : JSON.stringify(dataAry)
		         },
		         function(data,textStatus){
	...
	function check(obj){
        ...
        if(obj.value.match(re)==null){
          ...
          $.messager.alert('Warning!','请输入整数！!','warning');
        } 
	}
...
<div id="tt" class="easyui-tabs">
 <div title="审核信息"> 
 <div style="margin-bottom:10px;">信用额度：<input id="creditAmountTxt" type="text" value="${userAuditInfo.tempCreditAmount}" />元</div>
 <div style="margin-bottom:10px;">信用评分总和：<input id="creditTotle" type="text" value="${userAuditInfo.tempCreditScoreSum}" disabled="disabled"/></div>
 <table id="infoApprove" title="信息认证"   class="m_defined" width="100%">
		...
		<tr bgcolor="#FFFFFF">
		  ...
		  <td align="center">&nbsp;</td>
		  <td align="center">
		      <input id="16" name="creditGrade" type="text" value="${userAuditInfo.basicInfoApprove.tempCreditScore}" onkeyup="check(this)"/>
		  </td>
		  <td align="center">
		     <select id="sel16" name="cardSel">
		       <option value="0" <c:if test="${userAuditInfo.basicInfoApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		       <option value="1" <c:if test="${userAuditInfo.basicInfoApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		     </select>
		  </td>
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">
		    ...
		    <c:choose>
		     <c:when test="${userAuditInfo.cardUserApprove.cardPath!=null}">
		       <td align="center" >
		          <input id="1" name="creditGrade" type="text" value="${userAuditInfo.cardUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		       <td align="center">
		          <select id="sel1" name="cardSel">
		            <option value="0" <c:if test="${userAuditInfo.cardUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		            <option value="1" <c:if test="${userAuditInfo.cardUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		          </select>
		        </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		          <input id="1" name="creditGrade" type="text" value="${userAuditInfo.cardUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		       <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <c:choose>
		     <c:when test="${userAuditInfo.workUserApprove.cardPath!=null}">
		       <td align="center" >
		          <input id="2"  name="creditGrade" type="text" value="${userAuditInfo.workUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		        </td>
		        <td align="center">
		          <select id="sel2" name="cardSel">
		             <option value="0" <c:if test="${userAuditInfo.workUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		             <option value="1" <c:if test="${userAuditInfo.workUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		          </select>
		        </td>
		     </c:when>
		     <c:otherwise>
		      <td align="center" >
		        <input id="2" name="creditGrade" type="text" value="${userAuditInfo.workUserApprove.tempCreditScore}" disabled="disabled"/>
		      </td>
		      <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">
		    ...
		    <c:choose>
		     <c:when test="${userAuditInfo.creditReportUserApprove.cardPath!=null}">
		       <td align="center" >
		         <input id="4" name="creditGrade" type="text" value="${userAuditInfo.creditReportUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel4" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.creditReportUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.creditReportUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="4" name="creditGrade" type="text" value="${userAuditInfo.creditReportUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">
		   ...
		   <c:choose>
		     <c:when test="${userAuditInfo.earnUserApprove.cardPath!=null}">
		       <td align="center" >
		         <input id="3" name="creditGrade" type="text" value="${userAuditInfo.earnUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel3" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.earnUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.earnUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="3" name="creditGrade" type="text" value="${userAuditInfo.earnUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">
		   ...
		   <c:choose>
		     <c:when test="${userAuditInfo.houseUserApprove.cardPath!=null}">
		       <td align="center" >
		         <input id="5" name="creditGrade" type="text" value="${userAuditInfo.houseUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel5" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.houseUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.houseUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="5" name="creditGrade" type="text" value="${userAuditInfo.houseUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <c:choose>
		     <c:when test="${userAuditInfo.carUserApprove.cardPath!=null}">
		       <td align="center" >
		         <input id="7" name="creditGrade" type="text" value="${userAuditInfo.carUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel7" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.carUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.carUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="7" name="creditGrade" type="text" value="${userAuditInfo.carUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">
		   ...
		   <c:choose>
		     <c:when test="${userAuditInfo.marryUserApprove.cardPath!=null}">
		       <td align="center" >
		         <input id="8" name="creditGrade" type="text" value="${userAuditInfo.marryUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel8" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.marryUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.marryUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="8" name="creditGrade" type="text" value="${userAuditInfo.marryUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <c:choose>
		     <c:when test="${userAuditInfo.educationUserApprove!=null}">
		       <td align="center" >
		         <input id="12" name="creditGrade" type="text" value="${userAuditInfo.educationUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel12" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.educationUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.educationUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="12" name="creditGrade" type="text" value="${userAuditInfo.educationUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <c:choose>
		     <c:when test="${userAuditInfo.addressUserApprove.cardPath!=null}">
		       <td align="center" >
		         <input id="9" name="creditGrade" type="text" value="${userAuditInfo.addressUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel9" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.addressUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.addressUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="9" name="creditGrade" type="text" value="${userAuditInfo.addressUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">
		   ...
		   <c:choose>
		     <c:when test="${userAuditInfo.techUserApprove.cardPath!=null}">
		       <td align="center" >
		         <input id="6" name="creditGrade" type="text" value="${userAuditInfo.techUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel6" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.techUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.techUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="6" name="creditGrade" type="text" value="${userAuditInfo.techUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">&nbsp;</td>
		   <c:choose>
		     <c:when test="${userAuditInfo.videoUserApprove!=null}">
		       <td align="center" >
		         <input id="10" name="creditGrade" type="text" value="${userAuditInfo.videoUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel10" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.videoUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.videoUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="10" name="creditGrade" type="text" value="${userAuditInfo.videoUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <c:choose>
		     <c:when test="${userAuditInfo.phoneUserApprove.cardPath!=null}">
		       <td align="center" >
		         <input id="13" name="creditGrade" type="text" value="${userAuditInfo.phoneUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel13" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.phoneUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.phoneUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="13" name="creditGrade" type="text" value="${userAuditInfo.phoneUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">${userAuditInfo.userInfoPerson.sinaWeiboAccount}</td>
		   <c:choose>
		     <c:when test="${userAuditInfo.blogUserApprove!=null}">
		       <td align="center" >
		         <input id="14" name="creditGrade" type="text" value="${userAuditInfo.blogUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel14" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.blogUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.blogUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="14" name="creditGrade" type="text" value="${userAuditInfo.blogUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
		...
		<tr  bgcolor="#FFFFFF">
		  ...
		  <td align="center">&nbsp;</td>
		   <c:choose>
		     <c:when test="${userAuditInfo.fieldVisitUserApprove!=null}">
		       <td align="center" >
		         <input id="11" name="creditGrade" type="text" value="${userAuditInfo.fieldVisitUserApprove.tempCreditScore}" onkeyup="check(this)"/>
		       </td>
		        <td align="center">
		         <select id="sel11" name="cardSel">
		           <option value="0" <c:if test="${userAuditInfo.fieldVisitUserApprove.tempReviewStatus==0}">selected</c:if>>未通过</option>
		           <option value="1" <c:if test="${userAuditInfo.fieldVisitUserApprove.tempReviewStatus==1}">selected</c:if>>已通过</option>
		         </select>
		      </td>
		     </c:when>
		     <c:otherwise>
		       <td align="center" >
		         <input id="11" name="creditGrade" type="text" value="${userAuditInfo.fieldVisitUserApprove.tempCreditScore}" disabled="disabled"/>
		       </td>
		        <td align="center">
	...
	<div style="width:1000px; margin:15px 0; text-align:center;">
		<a class="easyui-linkbutton" onclick="confirm1()">审核通过</a>&nbsp;&nbsp;
		<a class="easyui-linkbutton" onclick="prompt3();">退回至待审</a>&nbsp;&nbsp;
		<a class="easyui-linkbutton" onclick="prompt4();">退回至初定</a>&nbsp;&nbsp;
		<a class="easyui-linkbutton" onclick="prompt1();">驳回</a>&nbsp;&nbsp;
		<a class="easyui-linkbutton" onclick="prompt2();">补充资料</a>&nbsp;&nbsp;
		<a href="#" onclick="javascript:reset()" class="easyui-linkbutton">重置</a>&nbsp;&nbsp;
		<a href="#" onclick="javascript:save()"class="easyui-linkbutton">保存并返回</a>
	</div>
	</sec:authorize>
	</sec:authorize>
