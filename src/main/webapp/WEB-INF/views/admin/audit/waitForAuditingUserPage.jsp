...
<script type="text/javascript">
	...
	function formatterOperation(val, row) {
	  // var row = $('#waitAuditTbl').datagrid('getSelected');
		url='${ctx}/admin/audit/waitForAuditingDetailPageJsp?userId='+row.userId;
		return '<a href="'+url+'">审核</a>';
	}
	
	function search(){
		var queryParams = $('#waitAuditTbl').datagrid('options').queryParams;
		...
		$('#waitAuditTbl').datagrid('options').queryParams = queryParams;
		$("#waitAuditTbl").datagrid('reload');
	}
...
<body>
<table id="waitAuditTbl" title="待审列表" class="easyui-datagrid"
			url="${ctx}/admin/audit/waitAuditUserPage?auditId=0"
