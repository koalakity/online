...
public class AudtiUserViewExcel extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		setText(getCell(sheet, 0, 9), "审核状态");

		List<AccountUsersAdmin> list = (List<AccountUsersAdmin>) model.get("userList");
		for (int i = 0; i < list.size(); i++) {
			String status = list.get(i).getMaterialReviewStatus().toString();
			String auditStatus="";
			if(status.equals("0")){
				auditStatus="待审";
			...
			setText(getCell(sheet, 1 + i, 9), auditStatus);
		}
